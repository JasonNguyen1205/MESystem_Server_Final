...
namespace MESystem.Data
{
    public class IfsService
    {
            ...
            GetShopOrderDetails(string orderNo, string releaserNo, string sequenceNo)
        {
            ...
            var result = await _context.ProductionPlans
        ...
        {
            string query = "select distinct  opt.CONTRACT," +
                                            "opt.ORDER_NO," +
                                            "opt.RELEASE_NO," +
                                            "opt.SEQUENCE_NO," +
                                            "opt.TOOL_ID," +
            ...
            var result = _context.ShopOrderOperTools
                                 .FromSqlRaw(query)
                                 .AsNoTracking()
                                 .ToList();
        ...
        {
            string query = "SELECT count (rowNumber) AS ToolsTotal " +
            ...
            var result = _context.ShopOrderOperToolsQty
                        .FromSqlRaw(query)
                        .ToList();
        ...
        {
            var result = _context.ManufToolDetails.Where(_ => false);
            if (!string.IsNullOrEmpty(barcode)) result = _context.ManufToolDetails.Where(s => s.Barcode == barcode);
            if (!string.IsNullOrEmpty(toolInstance)) result = _context.ManufToolDetails.Where(s => s.ToolInstance == toolInstance);
        ...
        {
            var result = _context.ShopOrds.Where(s => s.OrderNo == orderNo);
        ...
        public async void SaveChangesToIFS(string obejctId, string objectVersion, string column, string newValue, string table, string cv)
        {
            ...
            await _context.Database.ExecuteSqlRawAsync(queryUpdate);
        }
